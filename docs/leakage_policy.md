...
The canonical XGBoost model is then fit on full TRAIN using the verified Phase-3 representation. Project VALIDATION supports final comparison, paired bootstrap and a separate weighted sensitivity diagnostic. Native serialization is checked on TRAIN and VALIDATION without fitting. TEST is never predictively evaluated or used in those decisions. All relevant artifacts record test_set_evaluated=false. Paired bootstrap uses the same validation indices for both models and does not export customer predictions.

## Implemented Phase-6 calibration and threshold controls

For each fixed model, every TRAIN row receives exactly one OOF probability from a fold model whose preprocessing and estimator were fitted without that row. Final-model preprocessing is not reused inside these folds. A second five-fold CV splits only TRAIN OOF probabilities and labels to compare identity, sigmoid and isotonic; each mapping fits its probability-training subset and scores its held-out subset. The winner fits all TRAIN OOF pairs. Both final mappings are frozen before canonical VALIDATION scoring. VALIDATION and TEST are absent from calibrator fitting and selection interfaces.

The procedure is not a fully nested evaluation of all earlier model-selection decisions: OOF models have overlapping training sets, and XGBoost parameters were previously chosen using TRAIN. Calibration-selection CV is a development diagnostic, not an unbiased final performance estimate. VALIDATION is already a development set from Phase 5 and now selects the downstream model; it remains excluded from fitting either mapping. Paired intervals condition on these fixed models and this sample.

The selected model's max-KS threshold uses only TRAIN OOF reported probabilities. The 0.05–0.95 grid is TRAIN-only; VALIDATION receives only the frozen max-KS threshold and reference 0.50. No validation threshold search occurs. Threshold diagnostics can be optimistic because the final mapping was fitted on all TRAIN OOF labels. TEST remains sealed, with no probabilities, predictions, reliability, thresholds or bootstrap. No customer-level outputs are persisted.
